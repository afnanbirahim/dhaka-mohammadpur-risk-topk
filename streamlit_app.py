...
from app_utils import ensure_bundle

# Quiet SHAP noise
logging.getLogger("shap").setLevel(logging.ERROR)

# ---------- WHERE IS THE BUNDLE ----------
# Priority: explicit env (local/Colab) > downloaded from secrets (Streamlit Cloud)
BUNDLE_URL = st.secrets.get("BUNDLE_URL", "")
BASE = os.environ.get("MVP_BASE") or ensure_bundle(BUNDLE_URL)
...
def union_all_safe(geom_series):
    try: return geom_series.union_all()
    except Exception: return unary_union(list(geom_series))
...
def infer_dayparts(df):
    if "daypart" in df.columns: return sorted(df["daypart"].astype(str).unique()), "column"
...
for pth, label in [(GRID_GJ,"grid"),(MODEL_CAL,"model"),(META_JSON,"meta"),(FEATS_PARQ,"features")]:
    if not os.path.exists(pth): st.error(f"Missing {label}: {pth}"); st.stop()
...
if not months: st.error("No months found."); st.stop()
...
if dp_mode=="column":
    ...
    if dpcol in Df.columns: Df = Df[Df[dpcol]==1]
...
try:
    ...
    if joined.empty:
        ...
        if not use_folium:
            ...
            layer = pdk.Layer("GeoJsonLayer", data=joined.to_json(),
                              get_fill_color="[255,120,0, 80 + Math.floor(risk_norm*175)]",
                              get_line_color=[0,0,0,160], line_width_min_pixels=1, pickable=True)
            u = union_all_safe(joined.geometry); center = [float(u.centroid.y), float(u.centroid.x)]
            ...
            folium.GeoJson(joined[["geometry"]].to_json(),
                           name="TopK",
                           style_function=lambda x: {"color":"#ff7800","weight":1.5,"fillOpacity":0.35}).add_to(m)
...
if SECT_OK:
    ...
    if s.empty: st.info("No exported sectors for this day/daypart.")
    else:
        st.dataframe(s[["sector_id","n_hex","risk_sum","area_ha","dwell_min","sector_rank"]], use_container_width=True)
...
if sel_hex:
    ...
    if row_X.empty: st.info("No feature row for that hex.")
    else:
        if show_shap:
            try:
                import shap
                ...
                vals = sv[0]; idx = np.argsort(np.abs(vals))[::-1][:12]
...
c1,c2 = st.columns(2)
with c1:
    if os.path.exists(EVAL_SUM): st.download_button("Download eval summary", data=open(EVAL_SUM,"rb"), file_name="eval_summary.csv")
with c2:
    if os.path.exists(HRK_SW): st.download_button("Download HR@K sweep", data=open(HRK_SW,"rb"), file_name="hrk_sweep.csv")
